...
plot(theta,log(XRDCurve_simplematrix(lambda, d(mixing), zspace(mixing), suscdata(mixing,:), theta, stddevrad, thetaoffset)),theta,log(XRDCurve(lambda, d(mixing), zspace(mixing), suscdata(mixing,:), theta, stddevrad, thetaoffset)),theta,log(gidsl_simulation(:,2)'));

legend('simplematrix','original','gidsl');
